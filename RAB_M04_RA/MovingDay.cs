...
namespace RAB_M04_RA
{
    ...
    public class MovingDay : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            // propmt user to xelect Excel file
            ...
            Excel.Worksheet furnSetsWs = excel.Worksheets[1];
            Excel.Range furnSetsRange = (Excel.Range)furnSetsWs.UsedRange;
            Excel.Worksheet furnTypesWs = excel.Worksheets[2];
            Excel.Range furnTypesRange = (Excel.Range)furnTypesWs.UsedRange;

            // get row and column count
            int furnSetsRows = furnSetsRange.Rows.Count;
            int furnSetsColumns = furnSetsRange.Columns.Count;
            int furnTypesRows = furnTypesRange.Rows.Count;
            int furnTypesColumns = furnTypesRange.Columns.Count;

            // read Excel furniture set data into list
            List<List<string>> furnSetsData = new List<List<string>>();
            for (int i = 1; i <= furnSetsRows; i++)
            {
                List<string> rowData = new List<string>();
                for (int j = 1; j <= furnSetsColumns; j++)
                {
                    string cellContent = furnSetsWs.Cells[i, j].Value.ToString();
                    rowData.Add(cellContent);
                }
                furnSetsData.Add(rowData);
            }

            // read Excel furniture types data into list
            List<List<string>> furnTypesData = new List<List<string>>();
            for (int i = 1; i <= furnTypesRows; i++)
            {
                List<string> rowData = new List<string>();
                for (int j = 1; j <= furnTypesColumns; j++)
                {
                    string cellContent = furnTypesWs.Cells[i, j].Value.ToString();
                    rowData.Add(cellContent);
                }
            ...
            foreach (List<string> curFurnTypeArray in furnTypesData)
            {
                FurnitureType curFurnType = new FurnitureType(curFurnTypeArray[0], curFurnTypeArray[1], curFurnTypeArray[2]);
            ...
            foreach (List<string> curFurnSetArray in furnSetsData)
            {
                FurnitureSet curFurnSet = new FurnitureSet(curFurnSetArray[0], curFurnSetArray[1], curFurnSetArray[2]);
            ...
            using (Transaction t = new Transaction(doc))
            {
                ...
                foreach (SpatialElement curRoom in collector)
                {
                    ...
                    // 8. loop through furniture set data
